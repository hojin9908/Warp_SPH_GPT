"""Preprocessor example presets, migrated from the original Solv defaults.

Only input_gen and generator-based regression fixtures use these settings.
Solver entry points use input/Config*.py and load particle files instead.
The run defaults below keep the original generation validation reproducible.
"""
...
@dataclass
class CoupledGenerationConfig(GenerationConfig):
    """Original 3D SPH-DEM example material and geometry defaults."""
    dx: float = 0.02
    rho0: float = 1000.0
    dt: float = 1.0e-4
    n_steps: int = 9000
